...
\title{Elaborato Architettura dei Sistemi Digitali}
\author{Antonio Avolio - M63001352\\Davide Marcello - M63001375\\Christian Marescalco - M63001367\\Umberto Pier Rosario Caturano - M63001260\\Gruppo 57}
\date{}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth,height=\textheight,keepaspectratio]{\Edodici immagine.jpg}
\end{figure}
\vfill
{\let\newpage\relax\maketitle}
% {\centering \Huge \textbf{Progetto di Architettura dei}}
% \\{\centering \Huge \textbf{Sistemi Digitali}}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{\Ecinque cronometro.jpg}
    \caption{Sintesi su FPGA}
\end{figure}
\clearpage
...
\\I protocolli di trasmissione si dividono in 3 categorie:
\begin{enumerate}
    \item Protocolli sincroni: le 2 entità che vogliono comunicare presentano un riferimento temporale condiviso ed il trasferimento avviene all’interno di 1 o più cicli di clock.
    \item Protocolli Asincroni: le 2 entità presentano riferimenti temporali differenti, sui quali non possono essere fatte previsioni sulla fase iniziale. Tali protocolli prevedono una sincronizzazione non legata più a transizioni regolari dei fronti di un clock, ma a segnali di controllo/stato che le entità si scambiano in aggiunta ai dati, al fine di identificare l’inizio e la fine di un’operazione.
    \item Protocolli semi-sincroni: le 2 entità presentano un clock comune, così come nei protocolli sincroni, ma in aggiunta utilizzano un ulteriore segnale di controllo, detto wait. Tale segnale viene inviato dall’unità più lenta a quella più veloce quando non riesce ad elaborare i dati in un singolo colpo di clock. Il termine semi-sincrono deriva dal fatto che, seppur la trasmissione avviene tramite segnali di controllo/stato, questi vengono rilevati solo in presenza di un fronte del clock. (un protocollo di questo tipo è generalmente implementato nella comunicazione tra il processore e la memoria cache).
\end{enumerate}
I protocolli di handshake sono protocolli di tipo asincrono e si differenziano per il numero di eventi utilizzati nelle fasi di un trasferimento; si individuano 2 principali tipologie:
\begin{itemize}
    \item Handshake semplice: le 2 entità si sincronizzano solo all’inizio o alla fine di un trasferimento. Una volta sincronizzate, le 2 entità svolgono autonomamente le proprie operazioni e sono sempre pronte ad avviare una nuova trasmissione.
    \item Handshake completo: le 2 entità si scambiano eventi per definire l’inizio e la fine di ciascuna fase del protocollo, indipendentemente dalle rispettive velocità operative.
\end{itemize}
I protocolli di handshake, inoltre, si basano sulle seguenti ipotesi:
\begin{itemize}
    \item Le 2 entità lavorano sempre una come master e l’altra come slave.
    \item La trasmissione può essere avviata da entrambe.
    \item È l’entità più lenta a definire il ritmo della trasmissione.
    \item Non si pongono vincoli sulla velocità delle rispettive unità operative.
\end{itemize}
Per implementare un protocollo basato su handshake sono necessari almeno 2 segnali, tramite i quali codificare 2 o 4 eventi, a seconda che sia un protocollo semplice o completo.
\clearpage
...
\begin{figure}[ht!]
    \centering
    \includesvg[width=0.6\textwidth,height=\textheight]{\Esette sistema_completo.svg}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.5\textwidth,height=\textheight,keepaspectratio]{\Edieci blocco_instradamento.PNG}
    \caption{Schema blocco instradamento}
\end{figure}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.7\textwidth,height=\textheight,keepaspectratio]{\Edieci blocco_instradamento_vhdl.PNG}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.8\textwidth,height=\textheight,keepaspectratio]{\Edieci mux41vhdl.PNG}
    \caption{Mux 4:1}
\end{figure}
\clearpage
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.5\textwidth,height=\textheight,keepaspectratio]{\Edieci architetturacontrolunit1.PNG}
\end{figure}
\begin{figure}[ht!]
    \centering
...
\begin{itemize}
    \item Passo iniziale: si confrontano gli n bit più significativi del dividendo (dividendo parziale) con gli n bit del divisore: se il divisore è contenuto nel dividendo parziale, allora il quoziente parziale ($q_i$) sarà pari ad 1, 0 altrimenti (Il risultato delle operazioni di divisione parziale può essere al massimo 1, quindi in binario si ha un grande vantaggio).
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.4\textwidth,height=\textheight,keepaspectratio]{\Edodici immagine.jpg}
    \caption{Immagine iniziale}
\end{figure}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.4\textwidth,height=\textheight,keepaspectratio]{\Edodici immagine_compressa.jpg}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.5\textwidth,height=\textheight,keepaspectratio]{\Edodici vga_pinout.jpg}
    \caption{PIN}
\end{figure}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth,height=\textheight,keepaspectratio]{\Edodici display1.png}
    \caption{Display}
\end{figure}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth,height=\textheight,keepaspectratio]{\Edodici signal_timing_diagram.jpg}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.9\textwidth,height=\textheight,keepaspectratio]{\Edodici timingspec.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{\Edodici block_diagram.jpg}
...
\begin{figure}[ht!]
    \includegraphics[width=0.7\textwidth,height=\textheight,keepaspectratio]{\Edodici interfaccia_top_vhdl.PNG}
...
\begin{figure}[ht!]
    \includegraphics[width=0.8\textwidth,height=\textheight,keepaspectratio]{\Edodici constanti_vhdl.PNG}
    \caption{Costanti tempificazione}
\end{figure}
\\Vengono dichiarate le costanti relative al box dell’immagine:
\begin{figure}[ht!]
    \includegraphics[width=0.9\textwidth,height=\textheight,keepaspectratio]{\Edodici constanti2_bhdl.PNG}
    \caption{Costanti box immagine}
\end{figure}
\newpage
Vengono dichiarati i segnali che interagiscono internamente al controller:
\begin{figure}[ht]
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{\Edodici segnali_top_vhdl.PNG}
    \caption{Segnali interni}
\end{figure}
\\I componenti interni al top module sono:
\begin{figure}[ht]
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{\Edodici port_map_top_vhdl.PNG}
...
\begin{figure}[ht!]
    \includegraphics[width=0.75\textwidth,height=\textheight,keepaspectratio]{\Edodici contatore_orizz_vert_vhdl.PNG}
...
\begin{figure}[ht]
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{\Edodici generazione_sync_vhdl.PNG}
...
\begin{figure}[ht!]
    \includegraphics[width=0.5\textwidth,height=\textheight,keepaspectratio]{\Edodici moving_box_logic_1_vhdl.PNG}
    \caption{Process 1}
\end{figure}
\newpage
...
\begin{figure}[ht!]
    \includegraphics[width=1.1\textwidth,height=\textheight,keepaspectratio]{\Edodici moving_box_logic_2_vhdl.PNG}
    \caption{Process 2}
\end{figure}
\clearpage
...
\begin{figure}[ht]
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{\Edodici moving_box_logic_3_vhdl.PNG}
    \caption{Process 3}
\end{figure}
...
\clearpage
\begin{figure}[ht]
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{\Edodici assegnazione_indici_mem_vhdl.PNG}
...
\begin{figure}[ht!]
    \includegraphics[width=0.95\textwidth,height=\textheight,keepaspectratio]{\Edodici vga_pixel_assegnaz_vhdl.PNG}
    \caption{Process assegnazione pixel}
\end{figure}
\begin{figure}[ht!]
    \includegraphics[width=0.95\textwidth,height=\textheight,keepaspectratio]{\Edodici assegnazione_segnali_vga_vhdl.PNG}
...
\begin{figure}[ht!]
    \includegraphics[width=0.7\textwidth,height=\textheight,keepaspectratio]{\Edodici memoria1_vhdl.PNG}
\end{figure}
\\Per leggere il valore all’indirizzo dato da (indice\_riga, indice\_colonna) si utilizza il process:
\begin{figure}[ht]
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{\Edodici memoria2_vhdl.PNG}
    \caption{Memoria}
\end{figure}
...
\clearpage
\subsection{Esempi di risultati ottenuti}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.3\textwidth,height=\textheight,keepaspectratio]{\Edodici boardvga.jpg}
    \caption{Sintesi su FPGA}
\end{figure}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth,height=\textheight,keepaspectratio]{\Edodici fotoschermo1.jpg}
\end{figure}
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth,height=\textheight,keepaspectratio]{\Edodici fotoschermo2.jpg}
    \caption{Schermo con movimento dell'immagine}
\end{figure}
\newpage
\listoffigures
\end{document}